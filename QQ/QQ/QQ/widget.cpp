    ...
    ui(new Ui::Widget)
{
    ...
    ui->usrTblWidget->setEditTriggers(QAbstractItemView::NoEditTriggers);
...
void Widget::ReceiveMessage()
{
    ...
    int mytype;
    stream>>mytype;
    QString name,msg,recivename;//用户名 聊天内容
    ...
    case xChat:
        //发送者名称  接收者名称
        stream>>name>>recivename;
        if(recivename == myname)
        {
            chat *widget = new chat(nullptr,recivename,name);
            widget->setWindowIcon(QIcon(QString(":/image/2.jpg")));
            widget->setWindowTitle("和"+ name+"私聊中...");
            widget->show();
        }
        break;
    default:
...
{
    //点击右侧用户头像会调用这个函数
    if(ui->usrTblWidget->item(index.row(),0)->text() == myname)
    {
        QMessageBox::warning(this, tr("警告"), tr("不可以和自己聊天！！！"), QMessageBox::Ok);
        return;
    }
    else
    {
        QString  reciveName =ui->usrTblWidget->item(index.row(),0)->text();
        //创建私聊窗口
        chat *widget = new chat(nullptr,myname,reciveName);
        widget->setWindowIcon(QIcon(QString(":/image/1.jpg")));
        widget->setWindowTitle("和"+ reciveName+"私聊中...");
        widget->show();
